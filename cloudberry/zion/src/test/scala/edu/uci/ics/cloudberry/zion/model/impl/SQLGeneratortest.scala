...
class SQLGeneratorTest extends Specification {
  ...
  "SQLGenerator generate" should {
    ...
    "translate a simple unnest query" in {
      ...
      removeEmptyLine(result) must_== unifyNewLine(
        """|select hashtag as hashtag,count(*) as count
           |from twitter_ds_tweet t
           |where t.hashtag is not null and t.create_at >= '2016-01-01 00:00:00.000' and t.create_at < '2016-12-01 00:00:00.000' and lower(t.text) like '%zika%' and lower(t.text) like '%virus%' and t.geo_tag->"$.stateID" in ( 37,51,24,11,10,34,42,9,44 )
           |group by hashtag
           |order by count desc
    ...
    "translate a count cardinality query with filter without group by" in {
      ...
      removeEmptyLine(result) must_== unifyNewLine(
        """select count(*) as count from
          |(select *
          |from twitter_ds_tweet t
          |where t.create_at >= '2016-01-01 00:00:00.000' and t.create_at < '2016-12-01 00:00:00.000')""".stripMargin)
    }
    //11
    ...
    "translate a simple filter by time and group by time query max id" in {
      ...
      removeEmptyLine(result) must_== unifyNewLine(
        """
          ...
          |from twitter_ds_tweet t
          |where t.create_at >= '2016-01-01 00:00:00.000' and t.create_at < '2016-12-0100:00:00.000'
    ...
    "translate a max cardinality query with unnest with group by with select" in {
      ...
      removeEmptyLine(result) must_== unifyNewLine(
        """|select max(count) as max from
           |(select hashtag as hashtag,count(*) as count
           |from twitter_ds_tweet t
           |where t.hashtag is not null and lower(t.text) like '%zika%' and lower(t.text) like '%virus%' and t.create_at >= '2016-01-01 00:00:00.000' and t.create_at < '2016-12-01 00:00:00.000' and t.geo_tag->"$.stateID" in ( 37,51,24,11,10,34,42,9,44 )
           |group by hashtag
           |order by count desc
           |limit 10)""".stripMargin.trim)
    }
    ...
    "translate group by query having lookup with one join key" in {
      ...
      val result = parser.generate(query, Map(TwitterDataSetForSQL -> twitterSchema, populationDataSet -> populationSchema))
      removeEmptyLine(result) must_== unifyNewLine(
    ...
    "translate lookup one table with one join key" in {
      ...
      val result = parser.generate(query, schemaMap = Map(TwitterDataSetForSQL -> twitterSchema, populationDataSet -> populationSchema))
      removeEmptyLine(result) must_== unifyNewLine(
        """
    ...
    "translate multiple lookups inside group by state and count" in {
      ...
      val result = parser.generate(query, schemaMap = Map(TwitterDataSetForSQL -> twitterSchema, populationDataSet -> populationSchema, literacyDataSet -> literacySchema))
      removeEmptyLine(result) must_== unifyNewLine(
        """
