Run the setupScrapy.sh only once to setup the scrapy-splash environment

go to the scrapy directory: /cloudberry/src/main/resources/crawler/scripts/scrapy

Start new scrapy project by using command:
...
Append the contents of SplashConfig.txt in the directory: cloudberry/noah/src/main/resources/crawler to the settings.py file in the scrapy project directory. Make a shell script named move.sh containing following script:
...
SPLASH_URL = 'http://docker's Ip address:8050'
...
Go to spiders directory inside your scrapy project directory. Copy promedmail_spider.py from the directory cloudberry/noah/src/main/resources/crawler to your current directory using the cp command.
