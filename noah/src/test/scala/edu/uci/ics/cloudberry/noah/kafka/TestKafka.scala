...
class TestKafka extends Specification with Mockito {
  ...
  "Asterix Consumer Kafka" should {
    ...
    " subscribe in at least one topic" in {
      ...
      argument.getValue.size() > 0
    }

    "not call insert when record is empty" in {
      consumer.sendToAsterix(Config.Source.Zika, "", "", mockAsterix, mockClient, ConsumerRecords.empty())
      Mockito.verify(mockAsterix, Mockito.times(0)).insertRecord(any, any, any, any, any) must_== (())
