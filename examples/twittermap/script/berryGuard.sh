...
#                  0 * * * * ./path/to/berryGuard.sh [*DB host:port] [CB host:port] [TM host:port] [email to notify]
...
touchAsterixDB()
{
	...
	local result=$(curl -s --data-urlencode "statement=select count(*) from berry.meta;" http://${asterixDBHostPort}/query/service)
...
touchTwittermap()
{
	...
	local httpCode=$(curl -s -o /dev/null -w "%{http_code}" http://${twittermapHostPort})
