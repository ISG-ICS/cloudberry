  ...
  override def receive: Receive = {
    ...
    case append: AppendView =>
      ...
      if (isUpdating) {
        ...
        conn.postControl(queryParser.parse(append, schema)).map { result =>
...
object DataSetAgent {
  ...
  def props(schema: Schema, queryParser: IQueryParser, conn: IDataConn)(implicit ec: ExecutionContext) =
