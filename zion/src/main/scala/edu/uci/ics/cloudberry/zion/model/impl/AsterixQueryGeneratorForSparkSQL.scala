...
object AsterixImplForSparkSQL {


}


abstract class AsterixQueryGeneratorForSparkSQL extends IQLGenerator {
  ...
  protected def typeImpl: AsterixImplForSparkSQL
...
//  }
  protected def parseTextRelation(filter: FilterStatement, fieldExpr: String): String = {
    ...
    for (i <- 0 until (wordsArr.length - 1)){
      sb.append(s"lower($fieldExpr) ${typeImpl.fullTextContains} '${wordsArr(i)}' and ")
    }
    sb.append(s"lower($fieldExpr) ${typeImpl.fullTextContains} '${wordsArr(wordsArr.length - 1)}'")
    sb.toString()
  }
