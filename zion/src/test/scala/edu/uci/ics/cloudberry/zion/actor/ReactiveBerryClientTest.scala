...
//      val responseTime = 50
//
//      def childMaker(context: ActorRefFactory, client: BerryClient): ActorRef = worker.ref
//
...
//      sender.send(client, BerryClient.Request(hourCountJSON, NoTransform))
...
//      sender.send(client, BerryClient.Request(Seq((hourCountJSON, NoTransform))))
//      val askInfo = dataManager.receiveOne(5 seconds).asInstanceOf[DataStoreManager.AskInfo]
...
//      def childMaker(context: ActorRefFactory, client: BerryClient): ActorRef = worker.ref
//
//      val client = system.actorOf(BerryClient.props(mockParser, dataManager.ref, mockPlanner, Config.Default, childMaker, responseTime))
//
//      val query = mockParser.parse(hourCountJSON)
//
//      sender.send(client, BerryClient.Request(Seq((hourCountJSON, NoTransform))))
...
//
//      //send a new request
//      sender.send(client, BerryClient.Request(Seq((hourCountJSON2, NoTransform))))
//      Thread.sleep(250)
//      worker.reply(getRet(2))
//
//      sender.expectNoMsg()
